...
function startSentinel0(){
  ...
  fs.writeSync(sent0File,
               'port 26380\n' +
               'sentinel monitor mymaster 127.0.0.1 3680 1\n' + 
...
function startAllRedises(){
 ...
 function storeData(){
 //  clients['0'] = redis.createClient(3680, '127.0.0.1');
 //  clients['0'].on('error', function(err){
 //    console.log(err);
 //  });
 // clients['1'] = sentinelClient.createClient(26380, '127.0.0.1', {debug: false});
 // clients['1'].set("foo", "bar");
 // clients['1'].on('error', function(err, args){
 //   console.log('client error:' + err);
 // });
 // clients['1'].on('message', function(msg){
 //     console.log('client message: ' + msg);
 // });

//  setTimeout(killSomeStuff, 5000);
}
...
function killSomeStuff(){
  ...
  setImmediate(function(){clients['1'].get('foo', function(err, data){
  ...
  //   clients['0'].get('foo', function(err, data){
  //   console.log('A REPLY STAND OUT IN TEH CRAP!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!');
  //   if(err){
  //     console.log(err);
  //   }
